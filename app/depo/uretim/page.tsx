...
import { Package, PlusCircle, Trash2, ArrowDownCircle, ArrowUpCircle } from "lucide-react";
...
export default function UretimDepoPage() {
  ...
  useEffect(() => {
    ...
    getDepoHareketleri().then((data) => {
      setListe(data.filter((h) => h.depoTuru === "uretim"));
      setLoading(false);
    });
  }, []);
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-slate-800 text-xl font-bold flex items-center gap-2">
              <Package size={20} className="text-blue-500" /> Üretim Deposu
            </h1>
            <p className="text-slate-500 text-sm mt-0.5">Üretim depo giriş/çıkış hareketleri</p>
          </div>
            ...
            <form onSubmit={handleEkle} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="flex flex-col gap-1.5">
                  ...
                  <input name="urunAdi" value={form.urunAdi} onChange={handleChange} placeholder="ör. Hammadde A"
          ...
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <h2 className="text-slate-800 font-semibold text-base flex items-center gap-2">
                <Package size={18} className="text-blue-500" /> Hareketler
              </h2>
