...
export default function IsEmriForm({ onKaydet }: Props) {
  ...
  useEffect(() => { getUrunler().then(setUrunler); }, []);
  ...
  function handleUrunSec(e: React.ChangeEvent<HTMLSelectElement>) {
    ...
    if (secilen) setForm((prev) => ({ ...prev, urunAdi: secilen.urunAdi, birimFiyat: String(secilen.birimFiyat) }));
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.isEmriNo.trim() || !form.urunAdi.trim()) { setHata("İş emri numarası ve ürün adı zorunludur."); return; }
    if (!form.uretimAdedi || isNaN(uretim) || uretim <= 0) { setHata("Üretim adedi 0'dan büyük olmalıdır."); return; }
    const fireAdedi = isNaN(fire) ? 0 : fire;
    if (fireAdedi < 0 || fireAdedi > uretim) { setHata("Fire adedi geçersiz."); return; }

    setHata("");
    onKaydet({
      isEmriNo: form.isEmriNo.trim(), makinaNo: form.makinaNo.trim(),
      urunAdi: form.urunAdi.trim(), uretimAdedi: uretim, fireAdedi,
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
      <h2 className="text-slate-800 font-semibold text-base mb-5 flex items-center gap-2">
        <PlusCircle size={18} className="text-blue-500" /> Yeni İş Emri Kaydı
      </h2>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {urunler.length > 0 ? (
              <select onChange={handleUrunSec} defaultValue=""
...
interface InputAlaniProps {
  ...
  type?: string; inputMode?: React.HTMLAttributes<HTMLInputElement>["inputMode"]; placeholder?: string;
}
